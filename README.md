...
- **Adding items.** `ctrl+v` pastes a clipboard URL/path and appends it to
  ...
  does the same from outside mpv, over IPC - this plugin is the sole
  handler of the `mpv-add-item`/`mpv-add-item-next` messages it sends.
  Both funnel through the same core add logic: duplicate items are
  rejected (with an "Already in playlist" toast for the clipboard/manual
  case), a plain YouTube watch URL is never accidentally expanded into
  its autoplay "up next" mix, and an actual YouTube *playlist* URL is
  expanded via `yt-dlp` into its individual videos.
